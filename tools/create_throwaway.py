...
def _build_playwright_proxy(proxy: str, sticky_session: Optional[str] = None) -> dict:
    ...
    Tự động inject sticky session token cho các residential proxy provider:
      - IPRoyal: username_session-TOKEN_lifetime-10m
      - Decodo/Smartproxy: username-session-TOKEN
      - Oxylabs: username-sessid-TOKEN
    """
    ...
    if _p.username:
        ...
        if sticky_session:
            if "iproyal" in proxy_lower:
                username = f"{username}_session-{sticky_session}_lifetime-10m"
            elif any(h in proxy_lower for h in ("decodo", "smartproxy")):
                # Decodo/Smartproxy: user-session-TOKEN (port 10001 = sticky residential)
                username = f"{username}-session-{sticky_session}"
            elif "oxylabs" in proxy_lower:
                username = f"{username}-sessid-{sticky_session}"
...
def _make_sticky_token() -> str:
    """Tạo random sticky session token (dùng cho IPRoyal, Decodo, Oxylabs)."""
...
def create_google_account(
    ...
    device: tên thiết bị di động để giả lập (vd: "pixel7", "galaxy-s9").
            None = desktop Chrome.
    """
    try:
        ...
        try:
            ...
            print("❌ Cần cài patchright: pip install patchright && python -m patchright install chromium")
            return None
    ...
    with sync_playwright() as pw:
        ...
        if proxy:
            # Auto sticky session for residential proxy providers (each account = unique session)
            _proxy_lower = proxy.lower()
            _needs_sticky = any(h in _proxy_lower for h in ("decodo", "smartproxy", "iproyal", "oxylabs"))
            _sticky_tok = _make_sticky_token() if _needs_sticky else None
            _proxy_cfg = _build_playwright_proxy(proxy, sticky_session=_sticky_tok)
            launch_kwargs["proxy"] = _proxy_cfg
            _sticky_info = f" [sticky:{_sticky_tok}]" if _sticky_tok else ""
            print(f"  🌐 Proxy: {_proxy_cfg['server']}{_sticky_info}")
        browser = pw.chromium.launch(**launch_kwargs)

        # Resolve device descriptor for mobile emulation
        # Exclude 'default_browser_type' — not a valid new_context() parameter
        _CTX_DEVICE_KEYS = {"user_agent", "viewport", "device_scale_factor", "is_mobile", "has_touch"}
        _device_desc: dict = {}
        _resolved_device = None
        if device:
            _pw_name = MOBILE_DEVICES.get(device, device)  # allow raw pw device name too
            _resolved_device = pw.devices.get(_pw_name)
            if _resolved_device:
                _device_desc = {k: v for k, v in _resolved_device.items() if k in _CTX_DEVICE_KEYS}
                print(f"  📱 Giả lập thiết bị: {_pw_name}")
            else:
                print(f"  ⚠️  Thiết bị '{device}' không tìm thấy, dùng desktop mặc định")

        if _device_desc:
            # Mobile: use device descriptor (user_agent, viewport, is_mobile, has_touch included)
        ...
        try:
            ...
            def click_next() -> None:
                """Click nút Next/Continue. Mobile overlay workaround: try tap() + JS dispatch."""
                for sel in ['button:has-text("Next")', 'button:has-text("Continue")',
                             'button[type="submit"]']:
                    btn = page.locator(sel)
                    if btn.count() == 0:
                        continue
                    # Remove any pointer-intercepting overlay (jsname="OQ2Y6" = reCAPTCHA shield)
                    try:
                        page.evaluate(
                            """document.querySelectorAll('[jsname="OQ2Y6"]')
                    ...
                    try:
                        ...
                        pass
                    # Fallback: touch tap (works better on mobile emulation)
                    try:
                        btn.first.tap(timeout=5000)
                        return
                    except Exception:
                        pass
                    # Last resort: direct JS click bypassing all event interception
                    try:
                        _esc = sel.replace("'", "\\'")
                        page.evaluate(
                            "() => { const b = document.querySelector('" + _esc + "'); if (b) b.click(); }"
                        )
                        return
                    except Exception:
                        pass
            ...
            def detect_step() -> str:
                ...
                if (any(d in url for d in SUCCESS_DOMAINS)
                    ...
                    return "done"
                # signup/error — Google blocked submission (reCAPTCHA / bot detection)
                if "signup/error" in url.lower():
                    return "signup_error"
                # OTP input check before phone (both may have tel input)
                for _osel in _OTP_SELS:
                    if page.locator(_osel).count() > 0:
                        return "otp"
                # devicephoneverification/verify → check for OTP input (no clicking here)
                if "devicephoneverification/verify" in url.lower():
                    for _osel in _OTP_SELS:
                        if page.locator(_osel).count() > 0:
                            return "otp"
                    return "phone_verify"  # Needs Send SMS click from phone handler
            ...
            for _iter in range(20):
                ...
                if step == "name":
                    ...
                    if pf.count() > 0:
                        ...
                        # No phone input field — could be:
                        # 1. Consent page: "Verify your phone number" + "Send SMS" button
                        #    (Google pre-set phone, just needs confirmation to send OTP)
                        # 2. QR code page (true bot-detection block)
                        snap(f"step_{_iter:02d}_no_phone_input")

                        # Try all ways to find + click "Send SMS" / consent buttons
                        _sms_btn_sels = [
                            'button:has-text("Send SMS")',
                            'button:has-text("Send")',
                            'button:has-text("Use phone number")',
                            'button:has-text("Verify by SMS")',
                            'button:has-text("Get code")',
                            '[jsname="LgbsSe"]:has-text("Send")',
                        ]
                        _sms_txt_sels = [
                            "Send SMS", "Use phone number", "Verify by SMS",
                            "Enter phone number", "phone number",
                        ]
                        switched = False
                        # Button-based click (more reliable than text=)
                        for btn_s in _sms_btn_sels:
                            el = page.locator(btn_s)
                            if el.count() > 0:
                                try:
                                    el.first.click(timeout=5000)
                                except Exception:
                                    try:
                                        el.first.tap(timeout=5000)
                                    except Exception:
                                        pass
                                time.sleep(2)
                                wait_stable(8_000)
                                switched = True
                                print(f"  ✅ Clicked '{btn_s}' — consent page accepted")
                                break
                        # Fallback: text-based click
                        if not switched:
                            for switch_text in _sms_txt_sels:
                                ...
                                if el.count() > 0:
                                    ...
                                    wait_stable(8_000)
                                    switched = True
                                    print(f"  ✅ Clicked text='{switch_text}'")
                                    break

                        if switched:
                            # After clicking Send SMS: phone input might appear OR we go to OTP directly
                            ...
                            if pf2.count() > 0:
                                ...
                                # OTP was sent directly (consent-only page) — continue to OTP step
                                print(f"  📨 SMS đã gửi qua consent page — tiếp tục chờ OTP")
                                continue  # let state machine detect "otp" step next
                        else:
                            # Try to detect if it's a real QR code page
                            _page_html = page.content().lower()
                            _has_qr = any(kw in _page_html for kw in ["qr", "scan", "barcode"])
                            if _has_qr:
                                print(f"  ✗ Google yêu cầu QR code — không thể bypass tự động")
                            else:
                                print(f"  ✗ Không tìm thấy nút Send SMS — trang không xác định")
                                snap(f"step_{_iter:02d}_unknown_phone_page")
                            return None
                    ...
                    print(f"  ✓ OTP: {otp}")
                    _otp_filled = False
                    for csel in _OTP_SELS + ['input[id="code"]', 'input[aria-label*="code" i]']:
                        try:
                            ...
                            pass
                    if not _otp_filled:
                        print(f"  ⚠️  Không tìm thấy OTP input field")
                ...
                elif step == "phone_verify":
                    # devicephoneverification/verify — two sub-states:
                    # A) "Send SMS" button visible → click it to trigger SMS send
                    # B) "This may take a few moments" loading screen → poll for OTP input
                    _pv_url = page.url
                    _pv_otp_sels = _OTP_SELS + [
                        'input[aria-label*="code" i]',
                        'input[aria-label*="Enter" i]',
                        'input[aria-label*="verification" i]',
                        'input[type="tel"]:not([name="phoneNumberId"])',
                    ]
                    _vsms = page.locator(
                        'button:has-text("Send SMS"), button:has-text("Send")'
                    )
                    if _vsms.count() > 0:
                        print(f"  📤 Xác nhận gửi SMS (verify page)...")
                        try:
                            _vsms.first.click(timeout=5000)
                        except Exception:
                            try:
                                _vsms.first.tap(timeout=3000)
                            except Exception:
                                pass
                        print(f"  ✅ Clicked Send SMS — chờ OTP input (tối đa 90s)...")
                    else:
                        # Already in "This may take a few moments" state — just poll
                        print(f"  ⏳ Đang chờ Google gửi SMS (polling OTP input)...")

                    # Poll up to 90s for OTP input or navigation
                    _pv_deadline = time.time() + 90
                    _pv_advanced = False
                    while time.time() < _pv_deadline:
                        time.sleep(6)
                        if any(page.locator(s).count() > 0 for s in _pv_otp_sels):
                            _pv_advanced = True
                            print(f"  ✅ OTP input xuất hiện!")
                            break
                        if page.url != _pv_url:
                            _pv_advanced = True
                            print(f"  ✅ Page đã navigate sang: {page.url[:60]}")
                            break
                        # Check for "Try Again" — means loading timed out, click it
                        _try_again = page.locator('a:has-text("Try Again"), button:has-text("Try Again")')
                        if _try_again.count() > 0:
                            print(f"  🔄 Thấy 'Try Again' — thử lại...")
                            try:
                                _try_again.first.click(timeout=5000)
                            except Exception:
                                pass
                            time.sleep(5)
                            # After Try Again, look for Send SMS again or OTP
                            _vsms2 = page.locator('button:has-text("Send SMS"), button:has-text("Send")')
                            if _vsms2.count() > 0:
                                try:
                                    _vsms2.first.click(timeout=5000)
                                except Exception:
                                    pass
                                print(f"  📤 Clicked Send SMS (sau Try Again)...")
                        remaining = int(_pv_deadline - time.time())
                        if remaining > 0 and remaining % 18 == 0:
                            print(f"  ⏳ Vẫn chờ OTP input... ({remaining}s còn lại)")

                    if not _pv_advanced:
                        snap(f"step_{_iter:02d}_phone_verify_timeout")
                        print(f"  ✗ Timeout 90s — OTP input không xuất hiện")
                        return None
                    continue  # let state machine detect "otp" on next iter

                elif step == "signup_error":
                    # Google blocked submission at signup/error URL (bot detection / reCAPTCHA)
                    print(f"  ✗ Google chặn đăng ký (signup/error) — IP bị đánh dấu bot")
                    snap(f"step_{_iter:02d}_signup_error")
                    return None

                else:  # unknown
                    _cur_url = page.url
                    print(f"  ❓ Bước không xác định, thử Next... URL: {_cur_url[:60]}")
                    advanced = False
...
def main():
    ...
    parser.add_argument("--device", default=None,
                        choices=list(MOBILE_DEVICES.keys()),
                        metavar="|".join(MOBILE_DEVICES.keys()),
                        help="Gia lap thiet bi di dong (bot detection thap hon desktop). "
                             "Cac lua chon: " + ", ".join(
                                 f"{k}={v}" for k, v in MOBILE_DEVICES.items()
                             ))
    ...
    _device_label = f"{args.device} ({MOBILE_DEVICES.get(args.device, '')})" if args.device else "desktop"
    print(f"Thiet bi: {_device_label}")
