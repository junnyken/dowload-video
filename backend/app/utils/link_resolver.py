...
# Tracking params to strip (platform-agnostic)
_TRACKING_PARAMS = {
    "utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content",
    "fbclid", "gclid", "msclkid", "igshid", "ref", "ref_src", "ref_url",
    "feature", "si", "_branch_match_id", "s", "from", "app",
}
...
def _clean_expanded_url(expanded: str) -> str:
    """
    Strip tracking query parameters from expanded URLs.
    Keeps the path intact, removes utm_* / fbclid / igshid etc.
    """
    if "?" not in expanded:
        return expanded
    ...
    # For Douyin/TikTok, strip ALL params (they're unstable CDN tokens)
    if "douyin.com" in expanded or "tiktok.com" in expanded:
...
def normalize_url(url: str) -> str:
    """
    Normalize a URL:
      1. Strip tracking params (utm_*, fbclid, igshid, etc.)
      2. Expand youtu.be → youtube.com/watch?v=
      3. Normalize reddit.app.link → reddit.com
      4. Strip trailing slashes for consistency
    Does NOT expand general short links (needs network) — use resolve_short_url for that.
    """
    if not url:
        return url
    ...
    m = re.match(r'https?://youtu\.be/([\w-]+)(?:\?(.*))?$', url, re.IGNORECASE)
    if m:
        vid = m.group(1)
        extra = m.group(2) or ""
        from urllib.parse import parse_qs
        qs = {k: v for k, v in parse_qs(extra).items() if k.lower() not in _TRACKING_PARAMS}
        base = f"https://www.youtube.com/watch?v={vid}"
        if "list" in qs:
            base += f"&list={qs['list'][0]}"
        return base

    # b23.tv → already handled by short-link resolver (needs network)
    # Just clean tracking params for known long-form URLs
    return _clean_expanded_url(url)
...
async def resolve_short_url_async(url: str) -> str:
    ...
    """
    # Apply static normalization first (youtu.be, tracking params)
