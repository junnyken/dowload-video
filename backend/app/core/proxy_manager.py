...
# ── Env vars ────────────────────────────────────────────────────────
# IPROYAL_PROXY may be a SINGLE proxy or a COMMA-SEPARATED POOL. With a pool we
# round-robin across healthy proxies and skip ones in a cooldown (marked bad on
# 407/exhausted/tunnel errors) → spreads bandwidth + exit-IP reputation so many
# concurrent users don't burn/flag one proxy. A single value behaves as before.
import hashlib as _hashlib

_PROXY_POOL: list = [p.strip() for p in os.getenv("IPROYAL_PROXY", "").split(",") if p.strip()]
# Keep the legacy single-value symbol = first proxy (other modules import it).
IPROYAL_PROXY: str = _PROXY_POOL[0] if _PROXY_POOL else ""
...
_PROXY_BAD_TTL = int(os.getenv("PROXY_BAD_TTL", "600"))  # cooldown after a proxy error


def _proxy_id(p: str) -> str:
    return _hashlib.md5(p.encode()).hexdigest()[:10]


def _proxy_is_cooled(p: str) -> bool:
    try:
        from app.core.redis_client import get_redis
        return bool(get_redis().get(f"proxy_bad:{_proxy_id(p)}"))
    except Exception:
        return False


def mark_proxy_bad(proxy: str, ttl: int = None) -> None:
    """Cool down a proxy after a 407/exhausted/tunnel error so the pool rotates
    away from it (effectively a no-op when only one proxy is configured)."""
    if not proxy:
        return
    try:
        from app.core.redis_client import get_redis
        get_redis().setex(f"proxy_bad:{_proxy_id(proxy)}", ttl or _PROXY_BAD_TTL, "1")
        print(f"[ProxyPool] proxy {_proxy_id(proxy)} cooled for {ttl or _PROXY_BAD_TTL}s")
    except Exception:
        pass


def pick_proxy() -> Optional[str]:
    """Round-robin a healthy proxy from the pool. Single-proxy → returns it.
    All cooled → still returns one (better to try than to have no proxy)."""
    if not _PROXY_POOL:
        return None
    ...
    try:
        from app.core.redis_client import get_redis
        start = int(get_redis().incr("proxy_rr")) % len(_PROXY_POOL)
    except Exception:
        start = 0
    n = len(_PROXY_POOL)
    for i in range(n):
        cand = _PROXY_POOL[(start + i) % n]
        if not _proxy_is_cooled(cand):
            return cand
    return _PROXY_POOL[start % n]
...
def get_scraperapi_proxy_url(country_code: str = "") -> str:
    """Return ScraperAPI HTTP proxy URL for yt-dlp --proxy, or '' if no active key."""
    return _pool_proxy(country_code)
...
_PLATFORM_RULES: list[tuple[str, ProxyTier]] = [
    ...
    # ─── YouTube: residential proxy + cookies (residential IP prevents cookie invalidation) ──
    ...
    (r"threads\.(net|com)", ProxyTier.RESIDENTIAL),   # Threads — Meta anti-bot
