...
def send_daily_summary_sync() -> bool:
    ...
    try:
        ...
        try:
            ...
            for platform, info in pool.items():
                total = info["total"]
                if total == 0:
                    continue
                min_days = info.get("min_days_left")
                if min_days is not None and min_days <= 30:
                    emoji = "🔴" if min_days <= 7 else "🟡"
                    cookie_lines.append(
                        f"  {emoji} <b>{platform.upper()}</b>: {total} cookies, hết hạn sớm nhất {min_days} ngày"
                    )
                else:
                    cookie_lines.append(
                        f"  ✅ <b>{platform.upper()}</b>: {total} cookies, OK"
                    )
        except Exception:
            cookie_lines.append("  • Không thể kiểm tra cookie")
        cookie_text = "\n".join(cookie_lines) if cookie_lines else "  • Chưa có cookie"

        # ── Spotify Artist summary (P5c) ─────────────────
        try:
            from app.core.spotify_artist_ops import get_artist_stats, get_top_artists
            a = get_artist_stats()
            exp_ok, exp_fail = a.get("expand_ok", 0), a.get("expand_fail", 0)
            exp_rate = round(exp_ok / (exp_ok + exp_fail) * 100, 1) if (exp_ok + exp_fail) else 100.0
            sc_ok, sc_fail = a.get("sc_resolve_ok", 0), a.get("sc_resolve_fail", 0)
            sc_fail_rate = round(sc_fail / (sc_ok + sc_fail) * 100, 1) if (sc_ok + sc_fail) else 0.0
            top_lines = "\n".join(
                f"    {i+1}. {name} ({score})" for i, (name, score) in enumerate(get_top_artists(n=3))
            ) or "    —"
            artist_block = (
                f"\n🎤 <b>Spotify Artist:</b>\n"
                f"  📨 Requests: {a.get('requests', 0)}\n"
                f"  ✅ Expand rate: {exp_rate}%\n"
                f"  🔁 Deduped: {a.get('deduped', 0)} track\n"
                f"  🎧 SoundCloud fail: {sc_fail_rate}%\n"
                f"  🏆 Top nghệ sĩ:\n{top_lines}\n"
            )
        except Exception:
            artist_block = ""
