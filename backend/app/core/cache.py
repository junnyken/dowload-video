...
Two-tier cache:
  L1 — Redis  (TTL: URL_RESULT_CACHE_TTL, default 10 min) — eliminates
       repeated Supabase queries for the same hot URL within a burst window.
  L2 — Supabase (TTL: CACHE_TTL_HOURS = 24 h) — authoritative source.
"""
...
# L1 Redis TTL — short enough that stale CDN URLs aren't served too long.
_L1_TTL: int = int(os.getenv("URL_RESULT_CACHE_TTL", "600"))  # 10 min
_L1_PREFIX = "vidcache:urlresult:"


# ── Redis L1 helpers ─────────────────────────────────────────────────

def _l1_key(normalized_url: str) -> str:
    return _L1_PREFIX + hashlib.sha256(normalized_url.encode()).hexdigest()[:24]


def _l1_get(normalized_url: str) -> Optional[Dict[str, Any]]:
    """Return cached result from Redis L1, or None on miss/error."""
    try:
        raw = get_redis().get(_l1_key(normalized_url))
        if raw:
...
def _l1_set(normalized_url: str, result: Dict[str, Any]) -> None:
    """Populate Redis L1 with a successful result."""
    try:
        get_redis().setex(_l1_key(normalized_url), _L1_TTL, json.dumps(result))
    except Exception:
        pass
...
def get_cached_result(url: str) -> Optional[Dict[str, Any]]:
    ...
    L1 (Redis, 10 min) → L2 (Supabase, 24 h).  L1 hit skips the DB query
    entirely, which is important for viral URLs requested simultaneously by
    many users.
    ...
    try:
        ...
        if response.data:
            ...
            if direct_url:
                ...
                # Populate L1 so the next burst request avoids Supabase.
                _l1_set(normalized, result)
                return result
